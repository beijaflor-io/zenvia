...
class Zenvia {
  ...
  send(message, cb, options) {
    ...
    try {
      ...
      assert(message.from.length <= 20, 'message.from needs to be shorter or equal to 20 characters');
      assert((message.from.length + 2 + message.msg.length) <= 1500, 'message.from and message.msg summed must be greater than or equal to 1500 characters.');
    } catch (err) {
      return cb(err);
    }
    ...
    request
      ...
      .end((err, res) => {
        ...
        if (!res || res.status !== 200) {
          ...
          err.cause = res.body || res.status || res;
          debug('Error with SMS request', message.id, err.message || err);
          options.log({
            error: true,
            cause: err,
